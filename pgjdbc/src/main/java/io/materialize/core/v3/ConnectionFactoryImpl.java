...
import io.materialize.core.SetupQueryRunner;
import io.materialize.core.SocketFactoryFactory;
import io.materialize.core.Utils;
import io.materialize.core.Version;
import io.materialize.hostchooser.CandidateHost;
import io.materialize.hostchooser.GlobalHostStatusTracker;
import io.materialize.hostchooser.HostChooser;
import io.materialize.hostchooser.HostChooserFactory;
import io.materialize.hostchooser.HostRequirement;
import io.materialize.hostchooser.HostStatus;
import io.materialize.jdbc.SslMode;
import io.materialize.sspi.ISSPIClient;
import io.materialize.util.GT;
import io.materialize.util.HostSpec;
import io.materialize.util.MD5Digest;
import io.materialize.util.PSQLException;
import io.materialize.util.PSQLState;
import io.materialize.util.ServerErrorMessage;
...
public class ConnectionFactoryImpl extends ConnectionFactory {
  ...
  private ISSPIClient createSSPI(PGStream pgStream,
    ...
    try {
      ...
      Class<ISSPIClient> c = (Class<ISSPIClient>) Class.forName("io.materialize.sspi.SSPIClient");
      ...
      throw new IllegalStateException("Unable to load io.materialize.sspi.SSPIClient."
  ...
  private PGStream enableSSL(PGStream pgStream, SslMode sslMode, Properties info,
    ...
    switch (beresp) {
      ...
      case 'S':
        ...
        // Server supports ssl
        io.materialize.ssl.MakeSSL.convert(pgStream, info);
        return pgStream;
  ...
  private void doAuthentication(PGStream pgStream, String host, String user, Properties info) throws IOException, SQLException {
    ...
    io.materialize.jre7.sasl.ScramAuthenticator scramAuthenticator = null;
    //#endif
            ...
            switch (areq) {
              ...
              case AUTH_REQ_SSPI:
                ...
                if (useSSPI) {
                  ...
                  io.materialize.gss.MakeGSS.authenticate(pgStream, host, user, password,
              ...
              case AUTH_REQ_SASL:
                ...
                //#if mvn.project.property.postgresql.jdbc.spec >= "JDBC4.1"
                scramAuthenticator = new io.materialize.jre7.sasl.ScramAuthenticator(user, password, pgStream);
