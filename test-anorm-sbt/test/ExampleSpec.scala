...
class ExampleSpec extends PlaySpec with OneAppPerSuite {
  ...
  object Util {
    ...
    def parser[T](
      ...
      value match {
        case json: org.materialize.util.PGobject => parseJson(f, columnName.qualified, json.getValue)
        case _=> {
          Left(
            TypeDoesNotMatch(
              s"Column[${columnName.qualified}] error converting $value to Json. Expected class to be[org.materialize.util.PGobject] and not[${value.asInstanceOf[AnyRef].getClass}"
            )
          )
        }
